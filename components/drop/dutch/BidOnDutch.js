...
import { makeDutchBid } from "./transactions";
import Loading from "../../general/Loading";
import ArtPreview from "../special/ArtPreview";
import ArrowButton from "../../general/ArrowButton";

const BidOnDutch = ({ close, defaultBid, drop, art, ended, user, id }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await tx(
        [
          fcl.transaction(makeDutchBid),
          fcl.args([
            fcl.arg(
              process.env.NEXT_PUBLIC_CONTRACT || "0x99ca04281098b33d",
              t.Address
            ),
            fcl.arg(parseInt(id, 10), t.UInt64),
